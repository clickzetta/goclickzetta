...
func (qd *execResponseData) parseSchema() {
	...
	for _, field := range qd.HTTPResponseMessage.HttpResponseMessageResultSet.MetaData.Fields {
		...
		// A column carries at most one of these, so the sub-object the server
		// sent is the one that decides the branch.
		switch {
		case field.FieldType.DecimalTypeInfo != nil:
			precision, err := strconv.ParseInt(field.FieldType.DecimalTypeInfo.Precision, 10, 64)
			if err != nil {
				logger.WithContext(nil).Errorf("invalid decimal precision %q for column %q: %v", field.FieldType.DecimalTypeInfo.Precision, field.Name, err)
				return
			}
			scale, err := strconv.ParseInt(field.FieldType.DecimalTypeInfo.Scale, 10, 64)
			if err != nil {
				logger.WithContext(nil).Errorf("invalid decimal scale %q for column %q: %v", field.FieldType.DecimalTypeInfo.Scale, field.Name, err)
				return
			}
		...
		case field.FieldType.CharTypeInfo != nil:
			...
			if err != nil {
				logger.WithContext(nil).Errorf("invalid char length %q for column %q: %v", field.FieldType.CharTypeInfo.Length, field.Name, err)
				return
			}
