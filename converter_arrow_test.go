...
func TestArrowToValueNestedTypes(t *testing.T) {
	...
	// VECTOR 走的是和 ARRAY 一样的列表解码,再加一个字符串兜底。
	t.Run("VECTOR_string_fallback", func(t *testing.T) {
		arr := buildArray(t, arrow.BinaryTypes.String, func(b array.Builder) {
			b.(*array.StringBuilder).Append("[1.0,2.0]")
		})
		dest := make([]interface{}, arr.Len())
		if err := arrowToValue(dest, execResponseColumnType{Type: "VECTOR"}, arr, time.UTC, false); err != nil {
			t.Fatalf("arrowToValue: %v", err)
		}
		if dest[0] != "[1.0,2.0]" {
			t.Errorf("dest[0] = %#v, want the raw string", dest[0])
		}
	})
