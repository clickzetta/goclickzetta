...
func TestGetResponseErrorCode(t *testing.T) {
	tests := []struct {
		...
		{"numeric errorCode ignored", `{"status":{"errorCode":60023}}`, ""},
		{"boolean errorCode ignored", `{"status":{"errorCode":true}}`, ""},
	}
...
func TestGetResponseErrorMessage(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			value, err := fastjson.Parse(test.json)
			if err != nil {
				t.Fatalf("bad test json: %v", err)
			}
			if got := getResponseErrorMessage(value); got != test.want {
				t.Errorf("getResponseErrorMessage() = %q, want %q", got, test.want)
			}
		})
	}
}
